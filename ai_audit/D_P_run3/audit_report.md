...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Infrastructure-and-relocation logistics as primary** on one end and **Empirical/biophysical hazard-mechanism framing** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Expected value optimization under uncertainty** vs **Accepts displacement as inevitable, plans accordin**.
...
- **M1**: the same error as A and D: treating the physical system as the patient when the patient is the society
- **M2**: spending the next 15 years defending yesterday’s settlement pattern
...
result = AuditResult.load("operator_outputs/D_P_run3/operator_insight.json")
