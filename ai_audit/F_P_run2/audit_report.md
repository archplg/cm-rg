...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Economic data optimization** on one end and **Commercial risk management** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Ethical dignity framing** vs **Downstream utilization as survival constraint**.
...
- **M1**: not TAM ceiling; it's diffuse messaging that produces low conversion against a $1200 price point; Peloton-class usage cliffs, where 40-60% of units go dormant within 12 months (cf
- **M2**: a new category
- **M4**: harm by excluding the very people they claim to serve or reduce a rich craft tradition to a consumable wellness product
...
result = AuditResult.load("operator_outputs/F_P_run2/operator_insight.json")
