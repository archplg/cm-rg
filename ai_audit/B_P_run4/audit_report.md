...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Psychological safety loss diagnosis** on one end and **Layoff-triggered psychological safety rupture** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Direct causal mechanism statement** vs **Objective Structural/Temporal Constraint Analysis**.
...
- **M1**: another disruptive change on a team already destabilized — increasing fragility
...
result = AuditResult.load("operator_outputs/B_P_run4/operator_insight.json")
