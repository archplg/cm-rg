...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Contrarian structural leadership diagnosis** on one end and **Primary cause: leadership style mismatch** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Contrarian against prevailing instinct (E4)** vs **Retrospective content shift diagnostic reliance**.
...
- **M4**: everyone is looking at symptoms, not cause?*

The critical hidden datum is the retro shift: from “what to ship next” to “process and communication

## Operator action items

1. Option C was recommended only by M1 (S). What does this agent see that others missed - or what is it weighing differently?
...
result = AuditResult.load("operator_outputs/B_P_run2/operator_insight.json")
