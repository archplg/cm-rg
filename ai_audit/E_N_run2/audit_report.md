...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Ex ante frontier capability licensing** on one end and **ex ante licensing control** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Establishing a single harmonized cross-sector lice** vs **Outward norm projection**.
...
- **M1**: more directly
- **M2**: where post-hoc remedies are inadequate; uses, but it is not sufficient as the primary frontier framework; becoming paperwork around irreversible capability diffusion
...
result = AuditResult.load("operator_outputs/E_N_run2/operator_insight.json")
