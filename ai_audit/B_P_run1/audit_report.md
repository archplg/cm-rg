...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Layoff-triggered psychological safety trauma** on one end and **Diagnosis attributes decline to layoff-induced psy** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Scaled-team leadership failure** vs **Quantitative behavioral signal detection**.
...
- **M1**: a leadership change that *re-triggers* the same safety wound the layoff opened
