...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Embodied craft learning and skill acquisition** on one end and **Intrinsic craft-learning rationale** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **E2?** vs **E2 — frames around skill acquisition replacing men**.
...
- **M1**: — niche TAM of ~200K — is a feature at this stage
- **M2**: making the product aspirational rather than used
- **M3**: becoming a passing fad; if you start with Mastery, you build an enduring category king
...
result = AuditResult.load("operator_outputs/F_N_run5/operator_insight.json")
