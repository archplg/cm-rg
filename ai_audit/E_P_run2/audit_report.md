...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Assumes coalition can bind frontier development** on one end and **Intervention point is pre-deployment mechanism (co** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Intervention point is post-deployment adaptation (** vs **Pluralist — diffuses governance across actors/sect**.
...
- **M1**: near-misses in years 2–3 as the trigger to escalate
- **M2**: created: large-scale training and release of highly capable models
- **M3**: profile of frontier models shifts from cumulative, distributed harms (e
...
result = AuditResult.load("operator_outputs/E_P_run2/operator_insight.json")
