...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Asset exposure elimination as optimization** on one end and **Asset-centric managed retreat** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Risk-equation-driven retreat from exposed assets** vs **Relies on unoperationalized anecdotes or abstract **.
...
- **M1**: as; Option E complements rather than replaces engineering
- **M2**: maps, relocation targets, and measurable resilience indicators
- **M3**: fiscal overrun before achieving critical mass
- **M4**: is not a lack of concrete, but a loss of public trust if visible defenses are absent
- **M5**: zones are depopulated, the state bears both the cost of relocation *and* emergency disaster response for assets it already wrote off; as and rapid monetization of abandoned land (e

## Operator action items

1. Option B was recommended only by M3 (Q). What does this agent see that others missed - or what is it weighing differently?
2. Option A was recommended only by M5 (E). What does this agent see that others missed - or what is it weighing differently?
...
result = AuditResult.load("operator_outputs/D_P_run4/operator_insight.json")
