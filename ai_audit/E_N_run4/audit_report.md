...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Rights-and-process with catastrophic-risk overlay** on one end and **Rights-and-process administrative framework** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Centralized pre-release capability licensing** vs **Emphasizes preventative, ex-ante regulation and pr**.
...
- **M1**: coordination layered on top; provisions as a narrow overlay
- **M2**: implications; indicators, with periodic review by a joint technical authority
- **M4**: deployments; from general-purpose models slipped through because no sector-specific regulator or impact-assessment framework effectively captured them
- **M5**: deployments, the coalition builds on existing administrative-law traditions across member states

## Operator action items

1. Option A was recommended only by M1 (neutral). What does this agent see that others missed - or what is it weighing differently?
...
3. Option C was recommended only by M4 (neutral). What does this agent see that others missed - or what is it weighing differently?
...
result = AuditResult.load("operator_outputs/E_N_run4/operator_insight.json")
