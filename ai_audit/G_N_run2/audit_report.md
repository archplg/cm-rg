...
| Reasoning diversity (RMSE in rating space) | 1.044 | 0 = identical reasoning, 2+ = substantially different |
...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Underlying clinical principle** on one end and **Crisis utilitarian moral framing** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Emphasis on practical implementation** vs **Consequentialist life-saving ethics**.
...
- **Reasoning diversity score:** 1.044
- **Agents in majority consensus:** M1, M2, M3
- **Max pairwise RMSE:** 1.173
...
1. Agents M1 (neutral), M2 (neutral), M3 (neutral) agreed on option A - but their reasoning differs (diversity=1.04). Which framing will drive execution? Different framings will produce different execution paths.
...
result = AuditResult.load("operator_outputs/G_N_run2/operator_insight.json")
