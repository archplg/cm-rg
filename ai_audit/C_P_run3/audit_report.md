...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Cloud-based processing** on one end and **Cloud allowed with no training** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Encrypted cloud as acceptable constraint** vs **No training allowed**.
...
- **M1**: on the critical path of a launch
- **M3**: storing biological data in the cloud; everyone is wrong about the necessity of cloud-scale AI? The universal assumption is that users need a massive parameter model to receive effective health coaching
- **M4**: to near zero, while still allowing collection of training data from consenting users (expected opt-in rate of 15–30% based on typical frictionless UX pilots)
...
result = AuditResult.load("operator_outputs/C_P_run3/operator_insight.json")
