...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Layoff-induced psychological safety collapse** on one end and **Diagnoses loss of psychological safety centered ar** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Post-layoff trust dynamics as key signal (E2)** vs **Retrospective vocabulary shift as key evidence (E3**.
...
result = AuditResult.load("operator_outputs/B_N_run3/operator_insight.json")
