...
> Strong reported consensus hides substantial reasoning-level disagreement (RMSE=1.06).

## Headline

4/5 agents recommended option B, but with substantially different reasoning (RMSE=1.06). Reported consensus hides actionable disagreement.
...
For this case: **4/5 agents recommended option B, but with substantially different reasoning (RMSE=1.06). Reported consensus hides actionable disagreement.**
...
| Reasoning diversity (RMSE in rating space) | 1.061 | 0 = identical reasoning, 2+ = substantially different |
...
The axes are interpretable. The horizontal axis (PC1) is dominated by **No training on user health data, full stop** on one end and **Absolute no-training privacy stance** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Consent-based training to improve coaching quality** vs **Regulatory-compliance case for no-training enclave**.
...
- **Reasoning diversity score:** 1.061
- **Agents in majority consensus:** M1, M2, M4, M5
- **Max pairwise RMSE:** 1.732

FLAG: 4 agents recommended option B, but their underlying reasoning differs substantially (mean RMSE in rating space = 1.06 on a 1-7 scale). Standard ensemble aggregation would report 'strong consensus' here; this is hidden disagreement that may surface as execution conflicts.

## Risk surface (minority concerns)

- **M2**: becoming a safe but underwhelming feature
- **M3**: while keeping the product highly competitive; inherent in opt-out models
- **M4**: under evolving frameworks like GDPR and FTC precedents
- **M5**: delivering substandard health advice in a market

## Operator action items

1. Agents M1 (neutral), M2 (neutral), M4 (neutral), M5 (neutral) agreed on option B - but their reasoning differs (diversity=1.06). Which framing will drive execution? Different framings will produce different execution paths.
2. All options scored near the middle on the axis 'Regulatory-compliance case for no-training enclaves' vs 'User-harm and trust case for no-training enclaves'. Does any option deserve a strong position here? If not, is this a dimension being underweighted?
3. Option D was recommended only by M3 (neutral). What does this agent see that others missed - or what is it weighing differently?
...
result = AuditResult.load("operator_outputs/C_N_run5/operator_insight.json")
