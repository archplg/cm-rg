...
| M5 | neutral | E | `moonshotai/kimi-k2.6` |
...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Layoff-driven psychological safety loss** on one end and **E1, E3 — layoff-triggered psychological safety dia** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **E2 — emphasizes team size and lifecycle phase** vs **Scaling-inflection focus without layoff rebuttal**.
...
- **M2**: early, or trusting leadership intent; get hidden until late
...
result = AuditResult.load("operator_outputs/B_N_run2/operator_insight.json")
