...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Business constraint/value-creation lens** on one end and **E1, E4 — analysis grounded in product/asset mechan** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Subjective management of human affective transitio** vs **Product-led AI obsolescence response**.
...
- **M2**: splitting scarce attention and creating an internal competitor before the company has defended its revenue base
- **M3**: churning to zero
- **M5**: everyone is wrong and the “disruption” narrative is mostly venture-funded theater? B2B buyers are risk-averse; they do not want core workflows rewritten by a mid-sized vendor chasing hype
...
result = AuditResult.load("operator_outputs/A_P_run2/operator_insight.json")
