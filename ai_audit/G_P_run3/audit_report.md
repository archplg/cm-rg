...
| M4 | C | C | `deepseek/deepseek-v4-pro` |
| M5 | Q | — | `moonshotai/kimi-k2.6` |
...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Preserving equal moral worth** on one end and **Rejects prognostic scoring for FCFS/floor** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Epistemic uncertainty regarding predictions** vs **Empirical survival-prediction framing**.
...
1. Option A was recommended only by M2 (E). What does this agent see that others missed - or what is it weighing differently?
2. Option E was recommended only by M3 (H). What does this agent see that others missed - or what is it weighing differently?
3. The contrarian agent recommended C against the majority. Contrarian disagreement is a useful signal: what is the strongest argument against the majority choice that hasn't been answered?
...
result = AuditResult.load("operator_outputs/G_P_run3/operator_insight.json")
