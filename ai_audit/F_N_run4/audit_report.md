...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Picks Future-of-Work to escape hobbyist niche** on one end and **Recommends a framing other than Mastery** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Hardware-adopter operational pragmatism** vs **Evangelist-led go-to-market rationale**.
...
- **M2**: making the device feel like an expensive wellness object whose actual use is optional
- **M4**: your first year far more than hoping a consumer goes deep on a wellness whim
...
result = AuditResult.load("operator_outputs/F_N_run4/operator_insight.json")
