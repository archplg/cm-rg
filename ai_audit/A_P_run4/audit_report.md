...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Reasons from crowd-psychology and contrarian intui** on one end and **Decision process focus** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Technical/systems mechanism framework** vs **Skeptical/contrarian to consensus framing (E2, E3)**.
...
- **M2**: becoming a thin system of record around which value migrates elsewhere
- **M4**: breaking workflows they depend on; an abandoned product line would be a betrayal of trust; on the most vulnerable
...
result = AuditResult.load("operator_outputs/A_P_run4/operator_insight.json")
