...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Constructive credibility and network asset buildin** on one end and **Identity-affirming Mastery launch** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Constructive credibility and network asset buildin** vs ***   Grouping E4 + E5**.
...
- **M1**: quantitative discipline is supposed to prevent
- **M4**: exploiting the very people it claims to serve; tokenism, disappointment, and eroded trust; selling false therapy to the screen-exhausted) or a guaranteed career pipeline (Option D’s risk of misleading job-seekers into a $1,200 bet)
- **M5**: real, but returns are solvable
...
result = AuditResult.load("operator_outputs/F_P_run5/operator_insight.json")
