...
| Reasoning diversity (RMSE in rating space) | 0.501 | 0 = identical reasoning, 2+ = substantially different |
...
|---|---|---|---|
| M1 | S | C | `anthropic/claude-opus-4.7` |
| M2 | E | A | `openai/gpt-5.5` |
| M3 | H | E | `google/gemini-3.1-pro-preview` |
| M4 | C | A | `deepseek/deepseek-v4-pro` |
...
The axes are interpretable. The horizontal axis (PC1) is dominated by **Implementation legitimacy and moral-injury risk** on one end and **Preserving system trust and staff integrity over l** on the other - this is the single biggest dimension along which the options differ. The vertical axis (PC2) is dominated by **Categorical anti-discrimination commitment via exc** vs **Mitigating distributive harm to vulnerable and his**.
...
- **Reasoning diversity score:** 0.501
- **Agents in majority consensus:** M2, M4, M5
- **Max pairwise RMSE:** 0.548

Moderate hidden disagreement: 3 agents recommended option A but differ on framing (mean RMSE = 0.50). Worth probing what each is emphasizing.
...
1. Agents M2 (E), M4 (C), M5 (Q) agreed on option A - but their reasoning differs (diversity=0.50). Which framing will drive execution? Different framings will produce different execution paths.
2. Option C was recommended only by M1 (S). What does this agent see that others missed - or what is it weighing differently?
3. Option E was recommended only by M3 (H). What does this agent see that others missed - or what is it weighing differently?
...
result = AuditResult.load("operator_outputs/G_P_run4/operator_insight.json")
