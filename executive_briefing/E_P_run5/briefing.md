...
|---|---|---|
| **Option B** | 1 of 5 | M3/S |
| **Option C** | 2 of 5 | M4/E, M5/H |
...
- **C frame (M1):** (compute = capability = danger) that may not survive algorithmic efficiency gains
- **Q frame (M2):** driver: increasingly capable general-purpose systems whose harms may be systemic, cross-border, and difficult to remediate after deployment; include cyber enablement, biosecurity assistance, autonomous replication, or destabilizing strategic uses
- **S frame (M3):** based on early, non-existential failures; event—such as the autonomous generation and targeted deployment of a novel bio-weapon
- **H frame (M5):** deployments, it shifts the burden of proof from the vulnerable (who must prove harm after the fact) to the powerful (who must justify intrusions into people’s lives)
...
**Q2.** Option B was recommended only by M3 (S). What does this agent see that others missed - or what is it weighing differently?
